...
class DoublyLinkedList {
  ...
  isPalindrome() {
    ...
    //run through to mid and compare nodes from 0-mid with nodes length-mid
    //this.head=this.tail
    let runnerFront=this.head;
    let runnerBack=this.tail;
    let count=1;
    // console.log(runnerFront.data)
    // console.log(runnerBack.data)
    
    while(runnerFront.data === runnerBack.data && count!==mid)
    {
      // console.log(runnerFront.data)
      // console.log(runnerBack.data)

      runnerFront=runnerFront.next;
      // console.log("RF "+runnerFront);
      runnerBack=runnerBack.prev;
      // console.log("RB " + runnerBack)
      count++;
  ...
  isNodeInLeftHalf(node) {
    ...
    //go from 0-mid comparing if its anywhere from mid-length
    let count=length;
    let runnerLeft=this.tail;
    let output=false;
      while(count!==mid)
      {
        runnerLeft=runnerLeft.prev;
        if(runnerLeft===node)
        {
          output=true;
        }
...
const testNode= new DLLNode(1)
  testNode.next=-7;
  testNode.prev=6;

console.log(testNode.data, testNode.prev, testNode.next)
const pali = new DoublyLinkedList().insertAtBackMany([1,2,3,3,2,1])
const paliSecond = new DoublyLinkedList().insertAtBackMany([1,2,3,2,1])

console.log(pali.isPalindrome())
console.log(paliSecond.isPalindrome())
console.log(unorderedList.isPalindrome())
console.log(unorderedList.isNodeInLeftHalf(testNode))
console.log(secondThreeList.isNodeInLeftHalf(testNode))
